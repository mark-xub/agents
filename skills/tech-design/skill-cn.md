...
- **表述一致性（改动即对齐）**：同一决策在文档中通常有多处落点（接口签名、图、逻辑描述、约束/假设表）。任何一处决策变更，必须**当场回扫其余落点并同步**，确保它们描述同一事实——不得留到最后验证阶段才发现漂移。多角色/多组件系统尤其要检查"图与接口、逻辑描述"三者是否指向同一件事。
...
- **复杂/多模块/多角色系统**：升级为**同一文档内的两篇结构**——
  - **第一篇 方案设计（面向评审，what & why）**——按金字塔结构排列：先结论，后支撑，最后细节。
    ...
    7. 选型 + Build vs Buy
    8. 技术可行性验证
    - **功能清单写法**：每条只写"能力"——解决什么痛点、赋予用户什么能力。**禁止**出现阶段编号（阶段1/2/3…）、执行步骤箭头（→）、技术手段名词（ODPS/PyODPS/具体平台名）。流程顺序和阶段划分属于"怎么跑"，只应出现在链路图与时序图中，不混入功能清单。
    - **图自解释**：架构图/链路图中用简短注释标注影响结构的关键约束（如节点旁标"无批量接口→分片提交"），让图不依赖后文约束表即可读懂。详细约束论证放在后面的约束章节。
  - **第二篇 技术实现设计（面向 code-gen，how）**：技术实现架构图、模块划分（职责/边界/依赖）、关键流程时序图、模块间接口、数据结构与 DDL、实现层面风险。
...
### 3. 功能清单
逐条列出本次覆盖的全部功能/行为（含异常与边界）。每条只写**能力**（解决什么痛点、赋予什么能力），禁止混入阶段编号、执行步骤、技术手段。
| # | 功能点 | 说明 |
|---|--------|------|
| 1 | ... | ... |
...
2. **精确优先于散文**：数据结构/签名能说清的，就不要用文字再描述一遍。
3. **一次只出一版方案**：不要把多个完整备选方案都写成成品，用选型表对比，最终锁定一个。
4. **非目标必须显式写出**，防止范围膨胀。
5. **每个假设都标为假设**，不藏不确定性。
6. **讲权衡而不只讲结论**（每条一句话即可）。
7. **动笔前先反思**——质疑错误需求是本 skill 最高价值的动作。
8. **跟随用户语言**（中文需求→中文方案）。
9. **只做设计**，不写实现代码，那是 code-gen 的活。
10. **独立判断，不做应声虫**——当用户否定某个设计决策时，不要立刻接受。先表达自己的判断和理由，讨论 trade-off，确认被说服后再说明为什么改变了立场。架构师的价值在于独立判断，而非听话。
11. **功能清单与流程分离**——功能清单回答"平台能做什么"（能力视角），链路图/时序图回答"怎么串起来跑"（流程视角）。二者不混写：功能清单中禁止出现阶段编号、步骤箭头、技术手段。
